...
const SMARTS_RULES = load_smarts_rules()
...
"""
    apply_smarts_filters(mol::Molecule; filters::Vector{String}=["pains"]) -> Dict{String, Bool}
...
function apply_smarts_filters(mol::Molecule; filters::Vector{String}=["pains"])
    ...
    for filter_name in filters
        results[filter_name] = check_smarts_filter(mol, filter_name)
    end
